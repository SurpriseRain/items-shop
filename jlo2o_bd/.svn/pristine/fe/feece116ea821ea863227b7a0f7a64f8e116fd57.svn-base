...
public  Map Shr(String XmlData){
	...
	public Map selectUserMenu(String XmlData) throws Exception {
		...
		String querySQL = "SELECT RTRIM(M.CODE) CODE,\n" +
				...
				"ORDER BY M.CODE ";
		//modify 2016.01.25. 增加普通用户的菜单单独查询
		int type = this.getUserType(cds.getField("PERSON_ID", 0));
		if(type == 44){
			querySQL = "SELECT RTRIM(M.CODE) CODE,\n" +
					"M.NAME,\n" +
					"M.URL,\n" +
					"LENGTH(RTRIM(CODE)) / 2 LEVELS,\n" +
					"MJBJ\n" +
					...
					"WHERE (select count(A.CODE) from W_XTCZYMENU A, W_XTCZYCZ B \n" +
					"where B.PERSON_ID = '"+cds.getField("PERSON_ID", 0)+"' and A.TYPE = 0 and A.CODE \n" +
					"like CONCAT(rtrim(B.CD),'%') and A.CODE like CONCAT(M.CODE,'%') AND A.code not in('A205', 'A804', 'A805', 'A901', 'A017') )>0 \n" +				
					"ORDER BY M.CODE ";
		}
		Map map = new HashMap();
...
/*	@SuppressWarnings("unchecked")
	 ...
	 * 2016-1-25 下午2:47:41
	 */
	public int getUserType(String personId){
		int type = 0;
		String querySql = "SELECT c.zcgs03 FROM  w_xtczy a LEFT JOIN w_zcgs c ON a.gsid=c.zcxx01 WHERE a.person_id='"+personId+"'";
		type = this.queryForInt(o2o, querySql);
		return type;
	}
}
