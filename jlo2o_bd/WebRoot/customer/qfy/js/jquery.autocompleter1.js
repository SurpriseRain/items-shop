...
;(function ($, window) {
    ...
    var guid = 0,
        ignoredKeyCode = [9, 17, 19, 20, 27, 33, 34, 35, 36, 37, 39, 44, 92, 113, 114, 115, 118, 119, 120, 122, 123, 144, 145],
    ...
    function _build($node, opts) {
        if (!$node.hasClass("autocompleter-node")) {
            // Extend options
            opts = $.extend({}, opts, $("#qfySearch").data("autocompleter-options"));
            ...
            $("#qfySearch").addClass("autocompleter-node")
                 .after(html);

            var $autocompleter = $("#qfySearch").next(".autocompleter").eq(0);

            // Set autocomplete to off for warn overlay
            var originalAutocomplete = $("#qfySearch").attr("autocomplete");
            $("#qfySearch").attr("autocomplete", "off");
    ...
    function _launch(data) {
        ...
        data.template = data.combine().template;
    ...
    function _response(response, data) {
    	response = response.data.resultList;
    ...
    function _getCache(url) {
        if (!url) { return; }
        var response = (cache[url] && cache[url].innerHTML) ? cache[url].innerHTML : false;
        return response;
    }
