...
public class OrderPj extends JLBill{
	...
	public Map<String, Object> updateByxsdd01(String XmlData) throws Exception {
		...
		public Map insertW_SPPJ(String XmlData){
			...
			try {
				...
				//for(int i=0;i<spxxList.size();i++){
					...
					execSQL(o2o, sql, row);
					//更新订单主线状态
					//modify 2016.01.26.  对主线状态的更新增加检查条件，如果订单的商品item数量与评论数量相同，更新主状态
					int itemCount = queryOrderItemCount(cds.getField("XSDD01", 0));
					int commentCount = queryOrderItemCommentCount(cds.getField("XSDD01", 0));
					if(commentCount == itemCount){
		 ...
		 * 2016-1-26 下午3:26:05
		 */
		private int queryOrderItemCount(String orderId){
			String querySql = "SELECT count(xsdd01) FROM w_xsdditem WHERE xsdd01='"+ orderId +"'";
		 ...
		 * 2016-1-26 下午3:26:37
		 */		
		private int queryOrderItemCommentCount(String orderId){
			String querySql = "SELECT count(sppj01) FROM W_SPPJ WHERE xsdd01='"+ orderId +"'";
			int count = queryForInt(o2o, querySql);
			return count;			
		}
}
