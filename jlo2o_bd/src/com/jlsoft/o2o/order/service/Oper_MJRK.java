...
public class Oper_MJRK extends JLBill{
		...
		public Map insert_RKD(HttpServletRequest request,
			...
			try {
				...
				Map ckdMap = pubService.selectCKDForRKD(cds.getField("CKDH", 0), cds.getField("ZCXX01", 0));
				//插入入库单
				...
				//更新入库总数量 
				// 20160108 齐俊宇 update 修改入库数量增加出库单条件
				String update_ckdsl="UPDATE w_ckditem, ( SELECT spxx01, sum(rksl) c FROM w_rkd LEFT JOIN w_rkditem ON w_rkditem.RKDH = w_rkd.RKDH WHERE ckdh = '"+cds.getField("CKDH", 0)+"' GROUP BY spxx01 ) t SET RKSL = t.c WHERE t.spxx01 = w_ckditem.spxx01 AND w_ckditem.CKDH = '"+cds.getField("CKDH", 0)+"'";
				...
				if(tool.getCurConf(4)==1)
				{
					...
					String query="SELECT SPCM01, SPXX04, SPCM03, SPCM02,(SELECT b.ZCXX02 FROM w_ckd a LEFT JOIN w_zcxx b ON a.SHGS = b.zcxx01 WHERE a.CKDH='"+cds.getField("CKDH", 0)+"') ZCXX02, (SELECT b.ZCXX08 FROM w_ckd a LEFT JOIN w_zcxx b ON a.SHGS = b.zcxx01 WHERE a.CKDH='"+cds.getField("CKDH", 0)+"') ZCXX08,(SELECT b.ZCXX55 FROM w_ckd a LEFT JOIN w_zcxx b ON a.SHGS = b.zcxx01 WHERE a.CKDH='"+cds.getField("CKDH", 0)+"') ZCXX55,barcode,(SELECT SPTP02 FROM w_sptp WHERE SPXX01 = w_spxx.SPXX01 AND sptp01 = '1') SPTP FROM w_spcm LEFT JOIN w_spxx ON w_spxx.spxx01 = w_spcm.spxx01 LEFT JOIN w_zcxx ON w_zcxx.zcxx01 = w_spcm.zcxx01 where spcm01 in ("+s+") or spcm01 in (select curCode from w_spcmbz where parentCode in("+s+"))";
					List list = queryForList(o2o, query);
			...
			public Map insert_XSDDCM(HttpServletRequest request,
				...
				try {
					...
					for(int i=0;i<smList.length;i++)
					{
						...
						{
							spcm=spcm+"'"+smList[i]+"'";
							barcode = barcode +"'"+smList[i].substring(5, 18)+"'";
						}else
						{
							spcm=spcm+",'"+smList[i]+"'";
							barcode = barcode +", '"+smList[i].substring(5, 18)+"'";
					...
					// 20151224 齐俊宇 update 修改查询ZCXX65的sql改为查询出仓的仓库类型为ZCXX65->CKLX BEGIN
						...
						String sql = "SELECT DISTINCT A.XSDD01, A.ZTID, B.PFD01 FXDH, A.XSDD19 SHR, A.XSDD20 SHDZ, A.XSDD21 SHRDH, A.XSDD26 PSFS, C.TYPE CKLX "
								...
								+ "LEFT JOIN W_GOODS H ON H.SPXX01 = G.SPXX01 "
								+ "WHERE F.DQXX01 = A.CITY AND A.XSDD01='"+cds.getField("XSDD01", 0)+"' AND H.BARCODE IN ("+barcode+")";
		...
		public Map GetProduceExist(String XmlData)throws Exception{
			...
			try{
				...
				if(cds.getField("uniqueCode",0).toString().substring(cds.getField("uniqueCode",0).length()-2, cds.getField("uniqueCode",0).length()).equals("00"))
				{
					...
					//失败
						//2016.1.20 增加追溯不存在的数据本地验证
						selectProduce(XmlData);
						Map map1 = selectProduce(XmlData);
		...
		//2016.1.20 安迅入库,打包本地串码验证,不包含parentCode,仅为单个串码
		public Map<String, Object> selectProduce(String XmlData) throws Exception{
			Map<String, Object> map = new HashMap<String, Object>();
			try {
				cds = new DataSet(XmlData);
				String uniqueCode=cds.getField("uniqueCode",0);
				//type为1是安迅入库 2是扫码打包
				String type = cds.getField("TYPE",0);
				if (type.equals("1")){
					String ckdh = cds.getField("CKDH",0);
					String sql = "SELECT COUNT(*)  from w_ckdcm WHERE CKDH = '"+ckdh+"' AND SPCM01 = '"+uniqueCode+"'";
					int i = queryForInt(o2o, sql);
					String spcm01 = (String) map.get("spcm01");
					if (i == 1) {
						map.put("STATE", "1");
					}else{
						map.put("STATE", "2");
					...
					String sql = "SELECT COUNT(*)  FROM w_xsddcm WHERE XSDD01 = '"+XSDD01+"' AND SPCM01 = '"+uniqueCode+"'";
					int i = queryForInt(o2o, sql);
					String spcm01 = (String) map.get("spcm01");
					if (i == 1) {
						map.put("STATE", "1");
